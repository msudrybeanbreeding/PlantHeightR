...
shp_test_crs<- function(data, shape){
  tryCatch({
    ...
    } else if(!isTRUE(sf::st_crs(shape) == sf::st_crs(data))) {
...
shp_test_crs_pc<- function(data, shape){
  tryCatch({
    ...
    } else if(!isTRUE(lidR::st_crs(shape) == lidR::st_crs(data))) {
...
## Selecting individual images to run - DSM
  ...
  imgFiles.dsm_area_name <- reactive({
    if (!is.null(input$dsm_folder_area)) {
      if (dir.exists(input$dsm_folder_area)) {
        files <- list.files(path = input$dsm_folder_area, pattern="*.tif$", full.names = T) 
        files <- sapply(files, function(x) gsub(".tif", "", basename(x)))
  ...
  observe({
    if (!is.null(imgFiles.dsm_area_name())) {
      updateSelectInput(session, "nameID",
                        label = "Select the File name:",
                        choices = basename(imgFiles.dsm_area_name()),
                        selected = basename(imgFiles.dsm_area_name())[1])
    }
  })
    ...
    tryCatch({
      if(!is.null(input$dsm_folder_area)) {
        if (dir.exists(input$dsm_folder_area)) {
          ...
          # DSM <- imgFiles.dsm_area()
          # DSM1 <- stack(DSM[selected_dsm_file()])
          
          # Find the filename in DSM_files that matches selected_dsm_file()
          DSM_filename <- imgFiles.dsm_area()[grep(selected_dsm_file(), imgFiles.dsm_area())]
          ...
          plot(DSM1, xlab="", ylab="",)
          plot(shape, col = "red", add = TRUE)
          
         # # Plot raster and shapefile together
         #  rasterVis::levelplot(DSM1, col.regions = colorPal, margin = FALSE) +
         #    latticeExtra::layer(sp::sp.polygons(sf::as_Spatial(shape_sp), col = "red"))
    ...
    tryCatch({
      if (!is.null(input$laz_folder_area.pc)) {
        if (dir.exists(input$laz_folder_area.pc)) {
          ...
          cloud_s <- readLAS(files = imgFiles.laz_area.pc()[1], select = "xyz")
          shape <- indPlots_2_area()
