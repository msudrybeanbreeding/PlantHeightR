...
shp_test_crs<- function(data, shape){
  tryCatch({
    ...
    if(!is.na(sf::st_crs(shape)$epsg)) {
      ...
      message("CRS data have been adjusted using shapefile CRS. ")
...
shp_test_crs_pc<- function(data, shape){
  tryCatch({
    ...
    if(!is.na(lidR::st_crs(shape)$epsg)) {
    ...
    if(!isTRUE(lidR::st_crs(shape) == lidR::st_crs(data))) {
  ...
  tryCatch(
    foreach(i = 1:length(dsm_list), 
            ...
            .combine='c') %dopar% {
              ...
              crs.result <- shp_test_crs(DSM, shape)
              DSM <- crs.result$data
              shape <- crs.result$shape
