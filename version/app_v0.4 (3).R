    ...
    results<- foreach(p = 1:nrow(shape),  #loop through plots numbers
                      ...
                      .combine = rbind) %dopar% {
                       ...
                       if (ExtMet_soil == "quantile"){
                          csm_rem2 <- quantile(DSM2.c,qthresh_soil)
  ...
  tryCatch(
    results <- foreach(k = 1:nrow(shape),  #loop through plots numbers
                       ...
                       .combine = rbind) %dopar% {
                         ...
                         if (ExtMet_soil == "quantile"){
                           EPH_rem2 <- quantile(EPH.Extract.soil, probs = qthresh_soil, na.rm=TRUE)
...
## Selecting individual images to run - DSM
  ...
  selected_laz <- reactive({
    if (!is.null(input$flight_select_laz)) {
      file.path(normalizePath(input$laz_folder), input$flight_select_laz)
    }
  })
        ...
        if (input$dsm_single){
          ...
          output$downloadData <- downloadHandler(
            filename = function() { paste0("ts_flights", "_",
                                           input$PH_engine,"_veg_",
                                           input$analysis_type,"_",
                                           input$qthresh, "_soil_",
                                           input$qthreshSoil_single,"_",
                                           input$analysis_type_soil,"_",
                                           input$qthresh_s, 
                                           ".csv") },
